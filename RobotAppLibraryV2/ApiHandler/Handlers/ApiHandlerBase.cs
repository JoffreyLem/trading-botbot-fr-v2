...
public abstract class ApiHandlerBase : IApiHandler, IDisposable
{
    ...
    public async Task<List<CalendarData>> GetCalendarAsync()
    {
        try
        {
    ...
    public async Task<Position?> GetCurrentTradeAsync(string comment)
    {
        try
        {
    ...
    public async Task<SymbolInfo> GetSymbolInformationAsync(string symbol)
    {
        try
        {
            ...
            {
                var data = AllSymbols.First(x => x.Symbol == symbol);
    ...
    {
        if (AccountBalance is null) AccountBalance = new AccountBalance();
